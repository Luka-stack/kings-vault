...
const Modal = ({ onClose, userId }: Props) => {
  const [exportFile, setExportFile] = useState<File | null>();

  const exportRef = useRef<any>();

  const onExport = () => {
    IpcPasswd.exportPasswds(
      exportFile!.path.replace(exportFile!.name, ''),
      userId
    );
    setExportFile(null);
  };

  const exportInputChanged = (event: React.ChangeEvent<HTMLInputElement>) => {
            ...
            <div className="mt-3 text-center">
              ...
              <div className="flex items-center mt-4 min-w-[25rem] px-4">
                ...
                <p className="ml-4 text-sm text-gray-900">
                  {exportFile
                    ? shortenName(exportFile.path.replace(exportFile.name, ''))
                    : 'Select file...'}
                </p>
                ...
                <input
                  ...
                  multiple
                  ref={exportRef}
                  onChange={(e) => exportInputChanged(e)}
                />
              </div>
            </div>
