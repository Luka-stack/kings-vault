...
import fs from 'fs';
import { User, UserRepository as OldRepo } from './database-old/entities/user';
import { BrowserWindow } from 'electron';
import { PasswordRepository } from './database-old/entities/password';
...
export class PersistentService {
  ...
  constructor(private readonly mainWindow: BrowserWindow, dbFile: string) {
  ...
  initDatabase(dbFile: string): sqlite.Database {
    ...
    let db = new sqlite.Database(dbFile);
    db.get(OldRepo.findByIdStmt(1), (err: Error | null, row: any) => {
      if (err) {
  ...
  initTables(db: sqlite.Database): void {
    db.parallelize(() => {
      this.userRepository.initTable();
      this.userRepository.createAnonymousUser();
  ...
  createUser(username: string, password: string, strength: string): void {
    ...
    this._db.serialize(() => {
      let stmt = OldRepo.createUserStmt(user);
      ...
      stmt = OldRepo.logInStmt(username, passwordHash);
  ...
  logIn(username: string, password: string): void {
    ...
    const stmt = OldRepo.logInStmt(username, passwordHash);
  ...
  updateUser(username: string, password: string, strength: string): void {
    ...
    this._db.serialize(() => {
      let stmt = OldRepo.updateUserStmt(username, passwordHash, strength);
  ...
  uupdatePreferences(
    ...
    this._db.serialize(() => {
      let stmt = OldRepo.updatePreferencesStmt(id, notifyStatus, notifyDays);
      this._db.run(stmt, (_result: any, err: Error | null) => {
        if (err) {
          // TODO: Implement error handling
